...
module.exports.getRepairAccItList = async (id) => {
  ...
  , case when expence_id in (2,3,4) then rt.img_po when expence_id in (6) then rt.img_fin end as image
  , img_po, img_inv, rt.acc_approve, fp.TUserName as acc_name, acc_acc
  ...
  left join tb_users upo On upo.id = rt.po_id
  left join tb_users ufin On ufin.id = rt.fin_id
