...
export default function FormFin() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        let repaireData = await axios.get(process.env.REACT_APP_SERVER_ENDPOINT + '/api/repair_list_fin/' + id, { withCredentials: true })
  ...
  const onFinish = async (values) => {
    ...
    try {
      let response = await axios.put(process.env.REACT_APP_SERVER_ENDPOINT + '/api/repair_list_fin/' + id, values, { withCredentials: true })
            ...
            <div className="admin">
              ...
              {fileNameFin && (
                <div className="image-repair">
                  <img src={process.env.REACT_APP_SERVER_ENDPOINT+'/public/image/repair/' + fileNameFin} width={300} height={300} />
                </div>
              )}
            </div>
