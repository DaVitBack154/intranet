...
export default function FormPO() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        let repaireData = await axios.get(process.env.REACT_APP_SERVER_ENDPOINT + '/api/repair_list_po/' + id, { withCredentials: true })
        ...
        repaireData.data.data[0].inv_date = inv_datetime ? moment(inv_datetime) : null
  ...
  const onFinish = async (values) => {
    ...
    try {
      let response = await axios.put(process.env.REACT_APP_SERVER_ENDPOINT + '/api/repair_list_po/' + id, values, { withCredentials: true })
              ...
              <Form.Item className="form-item-preview-img">
                {fileNamePo && (
                  <div className="image-repair">
                    <img src={process.env.REACT_APP_SERVER_ENDPOINT+'/public/image/repair/' + fileNamePo} width={300} height={300} />
                  </div>
                )}
              </Form.Item>
              <Form.Item className="form-item-preview-img">
                {fileNameInv && (
                  <div className="image-repair">
                    <img src={process.env.REACT_APP_SERVER_ENDPOINT+'/public/image/repair/' + fileNameInv} width={300} height={300} />
                  </div>
                )}
              </Form.Item>
            </div>
