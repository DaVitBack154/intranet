...
export default function FormIt() {
    ...
    useEffect(() => {
        const init = async () => {
            ...
            let lastTicketNoData = await axios.get(process.env.REACT_APP_SERVER_ENDPOINT + '/api/ticket/last?type_id=1', { withCredentials: true })
    ...
    const onFinish = async (values) => {
        ...
        let result = await axios.post(process.env.REACT_APP_SERVER_ENDPOINT + '/api/repair_list/it', values, { withCredentials: true })
        if (result.data.status) {
            ...
            let lastTicketNoData = await axios.get(process.env.REACT_APP_SERVER_ENDPOINT + '/api/ticket/last', { withCredentials: true })
            if (lastTicketNoData.data.data.length < 1) {
                let newTicketNo = dayjs().format('YYYY-MM-') + '0001'
                setnewTicketNo(newTicketNo)
            } else {
