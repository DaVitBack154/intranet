...
export default function FormBuildingAdmin() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        let repaireData = await axios.get(process.env.REACT_APP_SERVER_ENDPOINT + '/api/repair_list/building/' + id, { withCredentials: true })
        ...
        let topicData = await axios.get(process.env.REACT_APP_SERVER_ENDPOINT + '/api/m/topic?type_id=2', {
  ...
  const onFinish = async (values) => {
    ...
    try {
      let response = await axios.put(process.env.REACT_APP_SERVER_ENDPOINT + '/api/repair_list/building/' + id, values, { withCredentials: true })
